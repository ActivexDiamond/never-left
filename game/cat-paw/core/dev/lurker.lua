...
local function isdir(path)
    ...
    if not info then
    	info = love.filesystem.getInfo("src/" .. path)
    end
...
local function lastmodified(path)
    ...
    if not info then
    	info = love.filesystem.getInfo("src/" .. path, "file")
    end
    return info.modtime
end
...
function lurker.getchanged()
  local function fn(f)
    return f:match("%.lua$") and lurker.files[f:gsub("src/", "")] ~= lastmodified(f)
  end
...
function lurker.resetfile(f)
  lurker.files[f:gsub("src/", "")] = lastmodified(f)
end
